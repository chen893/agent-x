...
function lineTrimmedFallbackMatch(
  ...
  while (currentIndex < startIndex && startLineNum < originalLines.length) {
    currentIndex += originalLines?.[startLineNum]?.length ?? 0 + 1; // +1 for \n
    startLineNum++;
  }
  ...
  for (
    ...
    if (matches) {
      ...
      for (let k = 0; k < i; k++) {
        matchStartIndex += originalLines?.[k]?.length ?? 0 + 1; // +1 for \n
      }

      // Find end character index
      let matchEndIndex = matchStartIndex;
      for (let k = 0; k < searchLines.length; k++) {
        matchEndIndex += originalLines?.[i + k]?.length ?? 0 + 1; // +1 for \n
      }
...
function blockAnchorFallbackMatch(
  ...
  let currentIndex = 0;
  while (currentIndex < startIndex && startLineNum < originalLines.length) {
    currentIndex += originalLines?.[startLineNum]?.length ?? 0 + 1;
    startLineNum++;
  }
  ...
  for (let i = startLineNum; i <= originalLines.length - searchBlockSize; i++) {
    ...
    for (let k = 0; k < searchBlockSize; k++) {
      matchEndIndex += originalLines?.[i + k]?.length ?? 0 + 1;
    }
...
class NewFileContentConstructor {
  ...
  private updateProcessingState(newState: ProcessingState) {
    const isValidTransition =
      ((this.state as ProcessingState) === ProcessingState.Idle &&
        newState === ProcessingState.StateSearch) ||
      ((this.state as ProcessingState) === ProcessingState.StateSearch &&
  ...
  private isStateActive(state: ProcessingState): boolean {
    return (this.state & (state as number)) === (state as number);
  }
